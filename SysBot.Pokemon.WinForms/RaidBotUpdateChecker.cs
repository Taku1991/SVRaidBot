...
{
    public class RaidBotUpdateChecker
    {
        ...
        public static async Task<(bool UpdateAvailable, bool UpdateRequired, string NewVersion)> CheckForUpdatesAsync(bool forceShow = false)
        {
            ...
            bool updateAvailable = latestRelease.TagName != GetCurrentRaidBotVersion();
        ...
        private static string GetCurrentRaidBotVersion()
        {
            try
            {
                // Try to get SVRaidBot version via reflection
            ...
            {
                LogUtil.LogError($"Error getting RaidBot version: {ex.Message}", "RaidBotUpdateChecker");
            }
